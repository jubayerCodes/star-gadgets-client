...
import { useGetPublicProductsQuery, useGetListingFiltersQuery } from "@/features/products/hooks/useProducts";
import Image from "next/image";
import Link from "next/link";
import { IPublicProductCategory, ISearchBrand, ISearchProduct, ProductStatus } from "../../types/product.types";
import { ChevronLeft, ChevronRight, Loader2, PackageSearch, Search, ShoppingCart, X } from "lucide-react";
...
export default function ProductsContent() {
  ...
  const { data: filtersData } = useGetListingFiltersQuery();

  const results: ISearchProduct[] = data?.data?.products ?? [];
  const brands: ISearchBrand[] = filtersData?.data?.brands ?? [];
  const categories: IPublicProductCategory[] = filtersData?.data?.categories ?? [];
  ...
  const filterSections: FilterSectionConfig[] = [
    ...
    },
    {
      ...
      onChange: (next) => updateParams({ category: next.join(",") || undefined }),
      scrollable: true,
      singleSelect: true,
    },
    {
  ...
  return (
    <div className="container py-8 min-h-[60vh] overflow-x-hidden">
      {/* ── Breadcrumb ── */}
      <SiteBreadcrumb className="mb-5" items={[{ label: "Products" }]} />
