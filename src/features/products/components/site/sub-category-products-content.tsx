...
import { useGetSubCategoryProductsQuery, useGetSubCategoryFiltersQuery } from "@/features/products/hooks/useProducts";
...
function ProductCard({ product }: { product: ISearchProduct }) {
  const { _id: productId, title, slug, badges, featuredVariant } = product;
...
function SubCategoryNotFound({ slug }: { slug: string }) {
  return (
    <div className="container py-20 min-h-[60vh] flex flex-col items-center justify-center gap-4 text-center">
      ...
      <div>
        <p className="text-base font-medium text-foreground">Sub-category not found</p>
        <p className="text-sm text-muted-foreground mt-1">
          No sub-category matches <strong>&ldquo;{slug}&rdquo;</strong>. It may have been removed or renamed.
        </p>
      </div>
...
export default function SubCategoryProductsContent({ subCategorySlug }: { subCategorySlug: string }) {
  ...
  // ── Fetch products ──
  const { data, isLoading, isFetching, isError } = useGetSubCategoryProductsQuery({
    subCategorySlug,
    page,
    limit,
  ...
  // ── Fetch filters (cached, independent of product query) ──
  const { data: filtersData } = useGetSubCategoryFiltersQuery(subCategorySlug);

  const subCategoryMeta = data?.data?.subCategory;
  ...
  const updateParams = useCallback(
    (updates: Record<string, string | undefined>) => {
      ...
      router.push(`/sub-categories/${subCategorySlug}?${params.toString()}`);
    },
    [searchParams, router, subCategorySlug],
  );
  ...
  const handleClearAll = () => router.push(`/sub-categories/${subCategorySlug}`);
  ...
  const filterSections: FilterSectionConfig[] = [
    {
      type: "radio",
      label: "Sort By",
      radioGroupName: "sub-category-sortBy",
      options: [
  ...
  // ── Not found ──
  if (isError) return <SubCategoryNotFound slug={subCategorySlug} />;
    ...
    <div className="container py-8 min-h-[60vh] overflow-x-hidden">
      ...
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center gap-4">
        <div className="flex items-center gap-3">
          {isLoading && !subCategoryMeta ? (
            <div className="h-7 w-48 bg-muted animate-pulse rounded" />
          ) : (
            <h1 className="text-xl font-semibold text-foreground">
              {subCategoryMeta ? `${subCategoryMeta.title} Products` : "Sub-Category Products"}
            </h1>
          )}
        ...
        <form onSubmit={handleSearchSubmit} className="flex items-center sm:ml-auto w-full sm:w-auto">
          <div className="relative flex-1 sm:w-72">
            ...
            <input
              id="sub-category-search-input"
              type="text"
              placeholder={`Search in ${subCategoryMeta?.title ?? "sub-category"}…`}
              value={localSearch}
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  {hasActiveFilters
                    ...
                    : "No products are available in this sub-category right now."}
                </p>
              </div>
              {hasActiveFilters && (
                <button
