...
export default function CheckoutContent() {
  ...
  const form = useForm<CheckoutFormValues>({
    ...
    defaultValues: {
      ...
      shipping_firstName: user?.shippingAddress?.fullName?.split(" ")[0] ?? "",
      shipping_lastName: user?.shippingAddress?.fullName?.split(" ").slice(1).join(" ") ?? "",
      shipping_streetAddress: user?.shippingAddress?.addressLine ?? "",
      shipping_city: user?.shippingAddress?.city ?? "",
      shipping_district: user?.shippingAddress?.district ?? "",
      shipping_postcode: user?.shippingAddress?.zipCode ?? "",
      shipping_phone: user?.shippingAddress?.phone ?? "",
    },
  });
